...
const API_URL = "/api.php"; // Diarahkan langsung ke api.php
...
function goTo(view){
  if(view === 'admin' || view === 'user'){
    loadPrices().then(async () => {
      ...
      if(view === 'user'){ renderPriceList(); renderUserHistory(); }
    });
  } else {
    document.querySelectorAll('body > div[id^="view-"]').forEach(el => el.classList.add('hidden'));
    document.getElementById('view-' + view).classList.remove('hidden');
...
async function addNewItem() {
  ...
  try {
    const res = await fetch(`${API_URL}?action=add_item`, {
      ...
      body: JSON.stringify({ category_id: Number(category_id), name, price: Number(price) })
    });
    ...
    if (json.status === 'success') {
      ...
      alert(json.message || 'Gagal menambah barang.');
    }
  } catch (e) {
    console.error('addNewItem error', e);
    alert('Terjadi kesalahan koneksi ke server.');
  }
}
...
async function submitTransaction(event){
  ...
  if(!nama){ 
    showBanner(banner, 'error', 'Isi nama pemesan terlebih dahulu.'); 
    return; 
  }
...
// Render History Transaksi di Halaman User
function renderUserHistory(){
  const searchNama = (document.getElementById('user-search-nama').value || '').toLowerCase().trim();
  const tbody = document.getElementById('user-history-tbody');
  
  if(!searchNama){
    tbody.innerHTML = '<tr class="empty-row"><td colspan="4">Ketik nama pada kolom di atas untuk mencari history.</td></tr>';
    return;
  }
  
  const userFiltered = allData.filter(d => (d.nama||'').toLowerCase().includes(searchNama));
  
  if(userFiltered.length === 0){
    tbody.innerHTML = '<tr class="empty-row"><td colspan="4">Tidak ditemukan riwayat transaksi untuk nama tersebut.</td></tr>';
    return;
  }
  
  tbody.innerHTML = userFiltered.map(d => `
    <tr>
      <td style="white-space:nowrap; color:var(--text-dim); font-size:12px;">${formatDate(d.tanggal)}</td>
      <td>
        <div style="font-weight:600; color:var(--text);">${escapeHtml(d.senjata !== '-' ? d.senjata : '')} ${escapeHtml(d.peluru !== '-' ? '| ' + d.peluru : '')}</div>
        <div style="font-size:12px; color:var(--success);">${formatRupiah(d.uang)}</div>
      </td>
      <td>${badgeHtml(d.status)}</td>
      <td style="color:${d.status === 'ditolak' ? 'var(--danger)' : 'var(--text-dim)'}; font-size:12.5px;">
...
function renderTable(){
  ...
  if(filtered.length === 0){
    tbody.innerHTML = '<tr class="empty-row"><td colspan="11">Belum ada data transaksi.</td></tr>';
    return;
  }
  tbody.innerHTML = filtered.map((d, i) => `
    <tr>
      <td>${i+1}</td>
      <td style="font-weight:600; color:var(--text);">${escapeHtml(d.nama||'-')}</td>
      <td style="white-space:nowrap; color:var(--text-dim); font-size:12px;">${formatDate(d.tanggal)}</td>
...
function openModal(id){
  ...
  if(id){
    ...
    document.getElementById('m-nama').value = d.nama || '';
